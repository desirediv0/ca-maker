...
import { processAndUploadImage, uploadPDF } from "../middlewares/multer.middlerware.js";
...
/**
 * When the route uses uploadFiles.any(), req.files is a flat array.
 * This helper returns files matching a specific fieldname.
 */
const getFiles = (files, fieldname) =>
  Array.isArray(files) ? files.filter((f) => f.fieldname === fieldname) : [];
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      const newProduct = await prisma.product.create({
        data: {
          ...
          courseType: req.body.courseType || "lecture",
          attempts: req.body.attempts
            ? typeof req.body.attempts === "string"
              ? JSON.parse(req.body.attempts)
              : req.body.attempts
            : [],
          modes: req.body.modes
            ? typeof req.body.modes === "string"
              ? JSON.parse(req.body.modes)
              : req.body.modes
            : [],
          bookOptions: req.body.bookOptions
            ? typeof req.body.bookOptions === "string"
          ...
          additionalInfo: (() => {
            if (!req.body.additionalInfo) return null;
            try {
              const parsed = typeof req.body.additionalInfo === "string"
      ...
      // Process Digital File (PDF)
      if (getFiles(req.files, "digitalFile").length > 0) {
        try {
          console.log("📄 Processing digital file...");
          const pdfFile = getFiles(req.files, "digitalFile")[0];
          const pdfUrl = await uploadPDF(pdfFile);
          console.log(`✅ Digital file uploaded: ${pdfUrl}`);
          await prisma.product.update({
            where: { id: newProduct.id },
            data: { digitalFile: pdfUrl },
          });
        } catch (error) {
      ...
      // Process Sample Book PDF
      if (getFiles(req.files, "sampleBookFile").length > 0) {
        try {
          console.log("📄 Processing sample book file...");
          const sampleFile = getFiles(req.files, "sampleBookFile")[0];
          const sampleUrl = await uploadPDF(sampleFile);
          console.log(`✅ Sample book uploaded: ${sampleUrl}`);
          await prisma.product.update({
            where: { id: newProduct.id },
            data: { sampleBookUrl: sampleUrl },
          });
        } catch (error) {
          console.error("❌ Error uploading sample book file:", error);
        }
      }

      // Upload product images if provided
      const imageFiles = getFiles(req.files, "images");
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      const updatedProduct = await prisma.product.update({
        ...
        data: {
          ...
          ...(req.body.courseType !== undefined && { courseType: req.body.courseType }),
          ...(req.body.digitalEnabled !== undefined && {
            digitalEnabled: req.body.digitalEnabled === "true" || req.body.digitalEnabled === true,
          }),
          ...(req.body.sampleNotes !== undefined && { sampleNotes: req.body.sampleNotes }),
          ...
          ...(req.body.attempts !== undefined && {
            attempts: typeof req.body.attempts === "string" ? JSON.parse(req.body.attempts) : req.body.attempts,
          }),
      ...
      // Process Digital File (PDF) — delete old file before uploading new one
      if (getFiles(req.files, "digitalFile").length > 0) {
        try {
          console.log("📄 Processing digital file (update)...");
          if (product.digitalFile) {
            try { await deleteFromS3(product.digitalFile); } catch (e) { /* ignore */ }
          }
          const pdfFile = getFiles(req.files, "digitalFile")[0];
          const pdfUrl = await uploadPDF(pdfFile);
          await prisma.product.update({
          ...
          console.log(`✅ Digital file updated: ${pdfUrl}`);
        } catch (error) {
          console.error("❌ Error uploading digital file:", error);
        }
      }

      // Process Sample Book PDF — delete old file before uploading new one
      if (getFiles(req.files, "sampleBookFile").length > 0) {
        try {
          console.log("📄 Processing sample book file (update)...");
          if (product.sampleBookUrl) {
            try { await deleteFromS3(product.sampleBookUrl); } catch (e) { /* ignore */ }
          }
          const sampleFile = getFiles(req.files, "sampleBookFile")[0];
          const sampleUrl = await uploadPDF(sampleFile);
          await prisma.product.update({
          ...
          console.log(`✅ Sample book updated: ${sampleUrl}`);
        } catch (error) {
          console.error("❌ Error uploading sample book file:", error);
        }
      }

      // Remove sample book if explicitly cleared
      if (req.body.clearSampleBook === "true") {
        if (product.sampleBookUrl) {
          try { await deleteFromS3(product.sampleBookUrl); } catch (e) { /* ignore */ }
        }
        await prisma.product.update({
          where: { id: productId },
          data: { sampleBookUrl: null },
        });
      }
