...
export const addToGuestCart = async (productVariantId, quantity = 1) => {
    try {
        ...
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "development" ? "http://localhost:4000/api" : "https://camaker.in/api")}/public/products/variants/${productVariantId}`,
            {
                credentials: "include",
            }
        );
        ...
        const mergePromises = guestItemsToMerge.map(async (guestItem) => {
            try {
                ...
                const addResponse = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "development" ? "http://localhost:4000/api" : "https://camaker.in/api")}/cart/add`,
                    {
                        method: "POST",
                        headers: {
