...
export function ProductForm({
  ...
  // Sample book state
  const [sampleBookFile, setSampleBookFile] = useState<File | null>(null);
  const [sampleBookUrl, setSampleBookUrl] = useState<string>("");
  const [provideSampleBook, setProvideSampleBook] = useState(false);

  // Additional info repeater
  const [additionalInfoItems, setAdditionalInfoItems] = useState<{ title: string; value: string }[]>([]);
        ...
        try {
          ...
          if (response.data.success) {
            ...
            setProduct({
              ...
              // Course specific fields
              courseType: productData.courseType || "lecture",
              attempts: productData.attempts || [],
              modes: productData.modes || [],
              bookOptions: productData.bookOptions || [],
              digitalEnabled: productData.digitalEnabled || false,
              sampleNotes: productData.sampleNotes || "",
            });

            // Set digital file URL if exists
            if (productData.digitalFile) {
              setDigitalFileUrl(productData.digitalFile);
            } else if (productData.digitalPdfUrls && productData.digitalPdfUrls.length > 0) {
              setDigitalFileUrl(productData.digitalPdfUrls[0]);
            }

            // Set sample book URL if exists
            if (productData.sampleBookUrl) {
              setSampleBookUrl(productData.sampleBookUrl);
              setProvideSampleBook(true);
            }

            // Load additional info items
            if (productData.additionalInfo) {
              try {
                const parsed = typeof productData.additionalInfo === "string"
                  ? JSON.parse(productData.additionalInfo)
                  : productData.additionalInfo;
                if (Array.isArray(parsed)) setAdditionalInfoItems(parsed);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append("courseType", (product as any).courseType || "lecture");
      formData.append("digitalEnabled", String(product.digitalEnabled));
      formData.append("sampleNotes", product.sampleNotes || "");

      // Append digital file if selected
      if (digitalFile) {
        formData.append("digitalFile", digitalFile);
      }

      // Append sample book file if selected
      if (sampleBookFile) {
        formData.append("sampleBookFile", sampleBookFile);
      }
      // Signal to backend to clear sample book if unchecked and there was one
      if (!provideSampleBook && sampleBookUrl && !sampleBookFile) {
        formData.append("clearSampleBook", "true");
      }

      // Append additional info as JSON
      if (additionalInfoItems.length > 0) {
        const validItems = additionalInfoItems.filter(i => i.title.trim() || i.value.trim());
        formData.append("additionalInfo", JSON.stringify(validItems));
      } else {
        formData.append("additionalInfo", JSON.stringify([]));
      }
      formData.append("attempts", JSON.stringify((product as any).attempts || []));
      formData.append("modes", JSON.stringify((product as any).modes || []));
      formData.append("bookOptions", JSON.stringify((product as any).bookOptions || []));
      // Add SEO fields
            ...
            <div className="space-y-4 grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-4 rounded-lg border p-4 bg-gray-50 md:col-span-2">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="flex items-center gap-2 pt-8">
                    <Checkbox
                      id="digitalEnabled"
                      checked={(product as any).digitalEnabled}
                      onCheckedChange={(checked) =>
                        setProduct(prev => ({ ...prev, digitalEnabled: !!checked }))
                      }
                      className="h-5 w-5 border-gray-400"
                    />
                    <Label htmlFor="digitalEnabled" className="cursor-pointer">
                      Digital Product (PDF/Drive)
                    </Label>
                  </div>
                </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm font-medium truncate max-w-[200px]">
                              {digitalFile ? digitalFile.name : (
                                digitalFileUrl.split("/").pop() || "Existing Digital File"
                              )}
                            </span>
                            {digitalFileUrl && !digitalFile && (
                              <a
                                href={digitalFileUrl}
                                target="_blank"
                            ...
                            {digitalFile && <span className="text-xs text-green-600">New file selected</span>}
                          </div>
                        </div>
                        <Button
                          variant="ghost"
                ...
                <div className="mt-4">
                  <Label>Sample Notes (URL)</Label>
                  <Input
                    placeholder="https://drive.google.com/..."
                    value={(product as any).sampleNotes || ""}
                    onChange={(e) => setProduct(prev => ({ ...prev, sampleNotes: e.target.value }))}
                  />
                  <p className="text-xs text-muted-foreground">Link to sample PDF/Notes</p>
                </div>
                ...
                <div className="mt-4 border-t pt-4 space-y-3">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="provideSampleBook"
                      checked={provideSampleBook}
                      onCheckedChange={(checked) => {
                        setProvideSampleBook(!!checked);
                        if (!checked) { setSampleBookFile(null); }
                      }}
                      className="h-5 w-5 border-gray-400"
                    />
                    <Label htmlFor="provideSampleBook" className="cursor-pointer font-medium">
                      Provide Sample Book (PDF Download)
                    </Label>
                  </div>
                      ...
                      {!sampleBookFile && !sampleBookUrl ? (
                        ...
                        >
                          <input
                            type="file"
                            accept=".pdf"
                            id="sample-book-upload"
                            className="hidden"
                            onChange={(e) => {
                              if (e.target.files?.[0]) setSampleBookFile(e.target.files[0]);
                            }}
                          />
                          <UploadCloud className="h-8 w-8 text-gray-400 mb-2" />
                          <span className="text-sm font-medium text-gray-700">Click to upload Sample PDF</span>
                          <span className="text-xs text-gray-500">Max size 50MB</span>
                        </div>
                      ) : (
                        <div className="flex items-center justify-between p-3 bg-white border rounded-md shadow-sm">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="text-sm font-medium truncate max-w-[200px]">
                                {sampleBookFile ? sampleBookFile.name : (sampleBookUrl.split("/").pop() || "Existing Sample Book")}
                              </span>
                              {sampleBookUrl && !sampleBookFile && (
                                <a href={`https://desirediv-storage.blr1.digitaloceanspaces.com/${sampleBookUrl}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">
                                  View Current File
                                </a>
                              )}
                              {sampleBookFile && <span className="text-xs text-green-600">New file selected</span>}
                            </div>
                          </div>
                          <Button
                            variant="ghost" size="sm" type="button"
                            onClick={() => setSampleBookFile(null)}
                            className="text-red-500 hover:text-red-700 hover:bg-red-50"
                          >
                            <X className="h-4 w-4" />
                          </Button>
              ...
              <div className="space-y-4 rounded-lg border p-4 bg-gray-50 md:col-span-2">
                <div className="flex items-center justify-between border-b pb-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => setAdditionalInfoItems(prev => [...prev, { title: "", value: "" }])}
                    className="gap-1 text-sm"
                  >
                    <span className="text-lg leading-none">+</span> Add Row
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">E.g. Author, Edition, Language, etc. Displayed as a table on the product page.</p>
                {additionalInfoItems.length === 0 && (
                  <p className="text-sm text-gray-400 italic py-2">No additional info added yet.</p>
                )}
                <div className="space-y-2">
                  {additionalInfoItems.map((item, idx) => (
                    <div key={idx} className="flex gap-2 items-center">
                      <Input
                        placeholder="Title (e.g. Author)"
                        value={item.title}
                        onChange={(e) => setAdditionalInfoItems(prev => prev.map((r, i) => i === idx ? { ...r, title: e.target.value } : r))}
                        className="flex-1"
                      />
                      <Input
                        placeholder="Value (e.g. CA Mohit Kukreja)"
                        value={item.value}
                        onChange={(e) => setAdditionalInfoItems(prev => prev.map((r, i) => i === idx ? { ...r, value: e.target.value } : r))}
                        className="flex-1"
                      />
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
                        onClick={() => setAdditionalInfoItems(prev => prev.filter((_, i) => i !== idx))}
              ...
              <div className="space-y-4 rounded-lg border p-4 bg-gray-50">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Select badges to highlight this product (e.g. Featured, Bestseller)
                  </p>
