...
import { RiStarFill as Star, RiAlertLine as AlertCircle } from "react-icons/ri";
      ...
      <div>
        ...
        <div className="flex justify-center">
          {[1, 2, 3, 4, 5].map((rating) => (
            <Star key={rating} className={`h-8 w-8 cursor-pointer ${formErrors.rating ? "text-red-200 hover:text-red-400" : "hover:text-yellow-400"} ${reviewForm.rating >= rating ? "text-yellow-400" : "text-gray-300"}`} onClick={() => handleRatingClick(rating)} />
          ))}
        </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center">
                <div className="flex text-yellow-400 mr-2">
                  {[...Array(5)].map((_, i) => <Star key={i} className={`h-4 w-4 ${i < Math.round(product.avgRating || 0) ? "text-yellow-400" : "text-gray-300"}`} />)}
                </div>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="flex text-yellow-400 mt-1">
                        {[...Array(5)].map((_, i) => <Star key={i} className={`h-4 w-4 ${i < review.rating ? "text-yellow-400" : "text-gray-300"}`} />)}
                      </div>
                    </div>
